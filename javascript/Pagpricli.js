...
function aplicarFiltros(priceRange, yearRange, nameSearch) {
  ...
  // Filtrar por año
  if (yearRange) {
    filteredVehiculos = filteredVehiculos.filter(vehiculo => vehiculo.año === parseInt(yearRange));
  ...
  if (nameSearch) {
    filteredVehiculos = filteredVehiculos.filter(vehiculo =>
      vehiculo.nombreVehiculos.toLowerCase().includes(nameSearch.toLowerCase())
    );
...
async function fetchVehiculos() {
  try {
    const response = await fetch("https://import20240918001333.azurewebsites.net/api/VehicleApi");
    if (!response.ok) {
...
async function fetchFotoPorVehiculo(idVehiculo) {
  try {
    const response = await fetch(`https://import20240918001333.azurewebsites.net/api/Fotos/${idVehiculo}`);
    if (!response.ok) {
...
async function renderVehiculo(vehiculos) {
  ...
  for (const vehiculo of paginatedVehiculos) {
    ...
    let imagePath = vehiculo.fotos && vehiculo.fotos[0] 
    ...
    card.innerHTML = `
      ...
      <div class="card-info">
        <div class="vehicle-info">
          <p>${vehiculo.nombreVehiculos}<br>${vehiculo.año}</p>
          <p>${vehiculo.precio}</p>
        </div>
        <div class="buttons">
          <button class="eliminar" style="display: none;">Eliminar</button>
          <button class="actualizar" style="display: none;" onclick="actualizarVehiculo('${vehiculo.idVehiculo}')">Actualizar</button>
          <button class="mas-info">Más Información</button>
        </div>
      </div>
    `;
...
function aplicarFiltros(category, priceRange, inStock) {
  ...
  if (category !== 'all') {
    filteredVehiculos = filteredVehiculos.filter(vehiculo => vehiculo.categoria === category);
  }
  ...
  // Filtrar por stock
  if (inStock) {
    filteredVehiculos = filteredVehiculos.filter(vehiculo => vehiculo.enStock === true);
  }
